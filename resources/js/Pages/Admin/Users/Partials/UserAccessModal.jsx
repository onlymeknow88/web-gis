...
export default function UserAccessModal({ isOpen, setIsOpen, accessForm, handleAccessSubmit, selectedUser, layers = [] }) {
    if (!isOpen) return null;

    const [searchQuery, setSearchQuery] = useState('');
    const [currentPage, setCurrentPage] = useState(1);

    const layerIds = accessForm.data.layer_ids || [];

    const handleCheckboxChange = (layerId, checked) => {
        if (checked) {
            accessForm.setData('layer_ids', [...layerIds, layerId]);
        } else {
            accessForm.setData('layer_ids', layerIds.filter(id => id !== layerId));
        }
    };

    const selectAll = () => {
        // Select all layers that are currently matching the search filter
        const filteredIds = filteredLayers.map(l => l.id);
        const newLayerIds = Array.from(new Set([...layerIds, ...filteredIds]));
        accessForm.setData('layer_ids', newLayerIds);
    };

    const deselectAll = () => {
        // Deselect all layers that are currently matching the search filter
        const filteredIds = filteredLayers.map(l => l.id);
        const newLayerIds = layerIds.filter(id => !filteredIds.includes(id));
        accessForm.setData('layer_ids', newLayerIds);
    };
    ...
    const filteredLayers = useMemo(() => {
        return layers.filter(layer => 
            layer.display_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            (layer.geoserver_layer && layer.geoserver_layer.toLowerCase().includes(searchQuery.toLowerCase())) ||
            (layer.description && layer.description.toLowerCase().includes(searchQuery.toLowerCase()))
        );
    }, [layers, searchQuery]);

    const itemsPerPage = 5;
    const totalPages = Math.ceil(filteredLayers.length / itemsPerPage) || 1;
    ...
    const paginatedLayers = filteredLayers.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
            ...
            <div className="bg-white rounded-2xl border border-outline-variant shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-6 pt-4 pb-2">
                    <div className="bg-surface-container-low border border-outline-variant rounded-xl p-3">
                        <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider block">Pengguna</span>
                        <span className="text-xs font-bold text-on-surface block leading-tight">{selectedUser?.name}</span>
                        <span className="text-[10px] text-on-surface-variant font-mono mt-0.5 block">{selectedUser?.email}</span>
                    </div>
                </div>
                ...
                <form onSubmit={handleAccessSubmit} className="p-6 pt-2 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                        <label className="block text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">
                            Pilih Layer yang Diizinkan ({layerIds.length} terpilih)
                        </label>
                    ...
                    <div className="relative">
                        <input
                            type="text"
                            placeholder="Cari nama layer / deskripsi..."
                            value={searchQuery}
                    ...
                    <div className="border border-outline-variant rounded-xl overflow-hidden divide-y divide-outline-variant bg-surface-container-lowest min-h-[120px]">
                        {paginatedLayers.length === 0 ? (
                            <div className="p-8 text-center text-on-surface-variant italic text-xs">
                                {searchQuery ? 'Tidak ada layer yang cocok dengan pencarian.' : 'Tidak ada layer GIS aktif yang tersedia.'}
                            </div>
                        ) : (
                            paginatedLayers.map((layer) => {
                                const isChecked = layerIds.includes(layer.id);
                                return (
                                    <label
                                        key={layer.id}
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="text-[9px] text-on-surface-variant font-mono mt-0.5 truncate">
                                                {layer.geoserver_layer}
                                            </div>
                                            {layer.description && (
                                                <div className="text-[10px] text-on-surface-variant mt-1 line-clamp-2">
                                                    {layer.description}
                                                </div>
                                            )}
                                        </div>
                    ...
                    {accessForm.errors.layer_ids && (
                        <p className="text-[10px] text-destructive-red font-semibold">{accessForm.errors.layer_ids}</p>
                    )}
