...
export default function EditMarkerModal({
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const initialLong = parseFloat(editForm.data.longitude) || 120.9213;
            const initialLat = parseFloat(editForm.data.latitude) || -3.7893;
            const zoom = editForm.data.longitude ? 10 : 4;

            if (editForm.data.longitude && editForm.data.latitude) {
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl border border-outline-variant shadow-2xl max-w-2xl w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="px-6 py-4 border-b border-outline-variant flex justify-between items-center bg-surface-container-low">
                    <h3 className="font-bold text-on-surface text-sm uppercase tracking-wider">Edit Marker</h3>
                ...
                <form onSubmit={handleEditSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                />
                                {editForm.errors.longitude && <p className="text-[10px] text-destructive-red mt-1 font-semibold">{editForm.errors.longitude}</p>}
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                />
                                {editForm.errors.latitude && <p className="text-[10px] text-destructive-red mt-1 font-semibold">{editForm.errors.latitude}</p>}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select
                                value={editForm.data.layer_id}
                                onChange={(e) => editForm.setData('layer_id', e.target.value)}
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-on-surface-variant/70 mt-1 block font-bold italic">
                            * Klik pada peta di atas untuk memperbarui koordinat.
                        </span>
                    </div>
                    ...
                    <div className="col-span-1 md:col-span-2 flex justify-end gap-3 pt-4 border-t border-outline-variant">
                        ...
                        >
                            {editForm.processing ? 'Menyimpan...' : 'Perbarui Marker'}
                        </button>
                    </div>
                </form>
            </div>
